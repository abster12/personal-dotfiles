...
**MANDATORY: You MUST use subagents here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not dispatch subagents you are doing this wrong.**

**Pi host:** Claude's "Agent tool" / `subagent_type="general-purpose"` maps to Pi's `subagent` tool with `agent: "worker"` (write + bash). Launch every chunk in one turn so they run in parallel (`subagent` `workflowScript` + `runs.all`, or parallel `subagent` calls). Do not use `scout` — it is read-only and cannot write chunk files, which silently drops extraction results. Invoke this skill with `/graphify` or `/skill:graphify`.
...
Dispatch every chunk in one turn so they run in parallel. Sequential dispatch (one child, wait, next child) defeats the purpose.

**IMPORTANT - subagent type:** Always use Pi `subagent` with `agent: "worker"`. Do NOT use `scout` — it is read-only and cannot write chunk files to disk, which silently drops extraction results. `worker` has write and bash.

Concrete example for 3 chunks — one `subagent` call with `workflowScript`:
```
subagent({
  workflowScript: `
    const results = await runs.all([
      { key: 'chunk1', agent: 'worker', task: PROMPT_1 },
      { key: 'chunk2', agent: 'worker', task: PROMPT_2 },
      { key: 'chunk3', agent: 'worker', task: PROMPT_3 },
    ]);
    return results;
  `,
  async: true,
...
```
All chunks in one `runs.all`. Not three separate turns.
...
- If the file is missing, the subagent was likely dispatched as read-only (`scout`) — print a warning: "chunk N missing from disk — subagent may have been read-only. Re-run with worker." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure `agent: "worker"` is used.

Merge all chunk files into `.graphify_semantic_new.json`. **After each child completes, read the real token counts from the subagent result usage and write them back into the chunk JSON before merging** — the chunk JSON itself always has placeholder zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
